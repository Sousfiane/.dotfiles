...
#trap 'exit' SIGINT
  ...
  upgrade() {
    if [ -s ${path}repo.pkgs ]; then
      ...
      xfce4-terminal --role float_term -e "aur sync -c -u --noview && sudo pacman -Syu --noconfirm"
      echo "%{T7}0%{T-}" > ${path}status && >| ${path}repo.pkgs && >| ${path}aur.pkgs
    else
  ...
  vcs_upgrade() {
    ...
    [ -s ${path}aur-vcs.pkgs ] && xfce4-terminal -e bash -ci \
  ...
  clean() {
    xfce4-terminal --role float_term -e"yay -Sc"
  }
